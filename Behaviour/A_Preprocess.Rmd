...
subjlist <- unique(data_raw_clothing$subj)
for (subject in subjlist) {
  # Get the subject number from the file name
  subdata <- data_raw_clothing[data_raw_clothing$subj == subject, ]
  triallist <- unique(subdata$trial)
  Ntrial <- length(triallist)
  
  for (t in triallist) {
    trial_data <- subdata[subdata$trial == t, ]
    Nfix <-  unique(trial_data$fixnum)
    
    #list of variables
    # note: ROI: currently gazed location (1: upper left 2: upper right 3: down left 4: down right). 
    brandleft_fix <- sum(trial_data$roi == 1)
    brandright_fix <- sum(trial_data$roi == 2)
    valueleft_fix <- sum(trial_data$roi == 3)
    valueright_fix <- sum(trial_data$roi == 4)
    brandleft_time <- sum(trial_data$fixdur[trial_data$roi == 1])
    brandright_time <- sum(trial_data$fixdur[trial_data$roi == 2])
    valueleft_time <- sum(trial_data$fixdur[trial_data$roi == 3])
    valueright_time <- sum(trial_data$fixdur[trial_data$roi == 4])
    brand_left <- trial_data$upleftval[1]
    brand_right <- trial_data$uprightval[1]
    value_left <- trial_data$downleftval[1]
    value_right <- trial_data$downrightval[1]
