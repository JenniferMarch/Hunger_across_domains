...
# Load the CSV file
data_raw_clothing <- read.csv("../data/borlabel.csv")
...
for (k in subjlist) {
  # Get the subject number from the file name
  subject <- subjlist[k]
  subdata <- data[data$subj == subject, ]
  Ntrial <-  unique(subdata$trial)
  ...
  for (t in Ntrial) {
    ...
    brand_left <- trial_data$upleftval
    brand_right <- trial_data$uprightval
    value_left <- trial_data$downleftval
    value_right <- trial_data$downrightval
    RT <- trial_data$rt
    #choice <- 
    response <- choice #1: left; 0: right
    trial <- t
    tempdata[t, ] <- c(subject, brandleft_fix, brandright_fix, valueleft_fix, valueright_fix, brandleft_time, brandright_time, valueleft_time, valueright_time, brand_left, brand_right, value_left, value_right)
  }
  data_clothing <- rbind(data_discount_sated, vpfile)
