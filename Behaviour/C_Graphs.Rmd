...
data_social_combined <- data_social_combined %>%
  dplyr::mutate(choice2 = dplyr::recode(as.character(choice), "selfish" = 1, "prosocial" = 0))

data_discount_combined <- data_discount_combined %>%
  dplyr::mutate(choice2 = dplyr::recode(as.character(choice), "impulsive" = 1, "patient" = 0))

p1 <- create_quantile_plot(data_combined, "choice2", "condition", "RT", label_type = "default")
p2 <- create_quantile_plot(data_social_combined, "choice2", "condition", "RT", label_type = "social")
p3 <- create_quantile_plot(data_discount_combined, "choice2", "condition", "RT", label_type = "discount")
...
nutri_graphs <- vd_graphs(data_combined_neu, "nutri_diff2", "Nutri", 12, 1700, 2900, min(data_combined_neu$nutri_diff2), max(data_combined_neu$nutri_diff2))
nutri_graphs
# For preference graphs
pref_graphs <- vd_graphs(data_combined_neu, "pref_vd2", "Preference", 12, 1700, 2900, min(data_combined_neu$pref_vd2), max(data_combined_neu$pref_vd2))
pref_graphs
# For calorie graphs
cal_graphs <- vd_graphs(data_combined_neu, "cal_vd2", "Caloric", 12, 1700, 2900, min(data_combined_neu$cal_vd2), max(data_combined_neu$cal_vd2))
cal_graphs
...
```{r}
#calculate the value differences left - right
data_social_combined$self_vd2<-data_social_combined$self_left-data_social_combined$self_right
data_social_combined$ngo_vd2<-data_social_combined$ngo_left-data_social_combined$ngo_right

# For self graphs
self_graphs <- vd_graphs(data_social_combined, "self_vd2", "Self", 12, 1500, 2500, min(data_social_combined$self_vd2), max(data_social_combined$self_vd2))
self_graphs
# For ngo graphs
NGO_graphs <- vd_graphs(data_social_combined, "ngo_vd2", "NGO", 12, 1500, 2500, min(data_social_combined$ngo_vd2), max(data_social_combined$ngo_vd2))
NGO_graphs
...
```{r}
#calculate the value differences left - right
data_discount_combined$value_vd2<-data_discount_combined$value_left-data_discount_combined$value_right
data_discount_combined$time_vd2<-data_discount_combined$time_left-data_discount_combined$time_right

# For self graphs
imp_graphs <- vd_graphs(data_discount_combined, "value_vd2", "Amount", 12, 1300, 2400, min(data_discount_combined$value_vd2), max(data_discount_combined$value_vd2))
imp_graphs
# For ngo graphs
pat_graphs <- vd_graphs(data_discount_combined, "time_vd2", "Time", 12, 1300, 2400, min(data_discount_combined$time_vd2), max(data_discount_combined$time_vd2))
pat_graphs
