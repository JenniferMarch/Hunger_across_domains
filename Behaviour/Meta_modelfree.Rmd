...
for (data_name in data_interested) {
  ...
  dataset <- dataset[dataset$up_dwelldiff >0 & dataset$down_dwelldiff >0, ] # only consider the datapoint where both fixations are evaluated
...
if (sd(participant_betas$beta_UpFixation) < 1e-10 ||
    ...
    #aired_ttest[[data_name]] <- t.test(participant_betas$beta_UpFixation, participant_betas$beta_DownFixation,
     #  mu = 0, 
      # paired = TRUE,   
       #var.equal = TRUE,
       #conf.level = 0.95)
