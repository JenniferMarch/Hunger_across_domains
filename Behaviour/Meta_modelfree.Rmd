...
save(data_interested, GLM_model_formulas, glm_ind_results, aic_glm_ind, glmm_results, aic_glmm, file = file_name)
# aic_glm_ind suggest that modelX is the best model to explain choice for all datasets!
```
...
for (data_name in data_interested) {
  ...
  for (model_name in names(GLM_model_formulas)) {
    coef_summary[[data_name]][[model_name]] <- list() # similar to coef_data but has more layers
    coef_ttest[[data_name]][[model_name]] <- list() # for GLM ttest results (and then plot them)
    ...
    # ttest between parameter of interests
    ...
    if (model_name == "model1") {
      ...
      coef_ttest[[data_name]][[model_name]][["Beta_DwellDiff_comparison"]] <-
        ...
        # add variables to GLMM coef organized data frame for the visualization
    temp_GLMM_coef <- summary(glmm_results[[data_name]][[model_name]])
    new_GLMMcoef <- data.frame(
      task = factor(rep(data_name, each = nCoefModel)),
      predictor = factor(predictor_vars, levels = desired_order),
      mean = temp_GLMM_coef$coefficients[,"Estimate"],
      sd = temp_GLMM_coef$coefficients[,"Std. Error"],
      z_val = temp_GLMM_coef$coefficients[,"z value"],
      p = temp_GLMM_coef$coefficients[,"Pr(>|z|)"]
    )
    GLMMcoef_data[[model_name]] <- rbind(GLMMcoef_data[[model_name]], new_GLMMcoef)
  }
}

# save data 
file_name <- paste0("GLMresults_minTrial_", minTrial, ".Rdata")
save(data_interested, GLM_model_formulas, glm_ind_results, aic_glm_ind, glmm_results, aic_glmm, coef_data, coef_ttest, GLMMcoef_data, file = file_name)
```
...
## GLM plot
Coef_plot <-
  ggplot(coef_data[["model3"]], aes(x = predictor, y = t_val)) +
  ...
  labs(title = "GLM: Impact of attribute values and dwell time (Actual Dwell time with MODEL3)",
       x = "Predictors",
...
## GLMM plot
Coef_GLMM_plot <-
  ggplot(GLMMcoef_data[["model3"]], aes(x = predictor, y = z_val)) +
  geom_point(size = 3) +  # Add points for means
  facet_wrap( ~ task, ncol = 1, scales = "free_y") +  # Vertically stack each task's plot
  ylim(-5, 45) +  # Set y-axis range
  geom_hline(yintercept = 1.96, linetype = "dashed",linewidth = 1.5, color = "red") +  # Add horizontal line at t value of 2.16
  labs(title = "GLMM: Impact of attribute values and dwell time (Actual Dwell time with MODEL3)",
       x = "Predictors",
       y = "Coefficients (z-value)") +
  theme_minimal()
# }
print(Coef_GLMM_plot)
