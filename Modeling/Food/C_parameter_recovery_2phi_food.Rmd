...
pr_sim_addmJAGS_sp_2Phi <- function(parameters,attribute1A,attribute1B,attribute2A,attribute2B,subject,fixProps){
...
for (p in 1:nPostPred){
  ...
  for  (s in (unique(data_sated$subject))){
    ...
    for (t in 1:2){
      ...
      ntrialsi <- nrow(sub)
      #recode everything into taste vs. health choice
      tasteL <- (tasteAi>tasteBi) #whether impatient option was left
      ...
      fixProp1_s <- sub$foodleft_time/(sub$foodleft_time+sub$foodright_time+sub$nutrileft_time+sub$nutriright_time) #taste left
      fixProp2_s <- sub$foodright_time/(sub$foodleft_time+sub$foodright_time+sub$nutrileft_time+sub$nutriright_time)#taste right
      fixProp3_s <- sub$nutrileft_time/(sub$foodleft_time+sub$foodright_time+sub$nutrileft_time+sub$nutriright_time)#amount left
      fixProp4_s <- sub$nutriright_time/(sub$foodleft_time+sub$foodright_time+sub$nutrileft_time+sub$nutriright_time)#amount right
    
      fixProps <- matrix(c(fixProp1_s*(tasteL==T)+fixProp2_s*(tasteL==F),fixProp1_s*(tasteL==F)+fixProp2_s*(tasteL==T),
                   fixProp3_s*(tasteL==T)+fixProp4_s*(tasteL==F),fixProp3_s*(tasteL==F)+fixProp4_s*(tasteL==T)),ncol=4)
      #print(fixProps[,1])
      
      #rescale taste and health values to lie between 1 and 10 (as in Yang & Krajbich)
      healthA_s <- (1+((healthA_s-min(healthA_s))*(10-1))/(max(healthA_s)-min(healthA_s)))
      healthB_s <- (1+((healthB_s-min(healthB_s))*(10-1))/(max(healthB_s)-min(healthB_s)))
      tasteA_s <- (1+((tasteA_s-min(tasteA_s))*(10-1))/(max(tasteA_s)-min(tasteA_s)))
      tasteB_s <- (1+((tasteB_s-min(tasteB_s))*(10-1))/(max(tasteB_s)-min(tasteB_s)))
      ...
      tasteA <- tasteA_s[validTrials]
      tasteB <- tasteB_s[validTrials]
      healthA <- healthA_s[validTrials]
      healthB <- healthB_s[validTrials]
      subject <- subjecti[validTrials]
      fixProps <- fixProps[validTrials,]

      #simulate
      pr_Dat <- pr_sim_addmJAGS_sp_2Phi(parameters,tasteA,tasteB,healthA,healthB,subject,fixProps)
      if (t==1){
        simDat_JAGS_hungry <- rbind(simDat_JAGS_hungry, pr_Dat)
        if (p==nParamRecov){ #save trial info at the last iteration
          conflictTrials <- ((tasteA>tasteB)&(healthA<healthB))|((tasteA<tasteB)&(healthA>healthB))
          conflictTrials_hungry <- c(conflictTrials_hungry,conflictTrials)
        }
      } else {
        simDat_JAGS_sated <- rbind(simDat_JAGS_sated, pr_Dat)
        if (p==nParamRecov){ #save trial info at the last iteration
          conflictTrials <- ((tasteA>tasteB)&(healthA<healthB))|((tasteA<tasteB)&(healthA>healthB))
...
for (s in (unique(simDat_JAGS_hungry$subject))){
  ...
  for (t in 1:2){
    ...
    tasteA_s <- sub$attribute1A
    tasteB_s <- sub$attribute1B
    healthA_s <- sub$attribute2A
    healthB_s <- sub$attribute2B
    ntrialsi <- nrow(sub)
...
ddmData <- list('N'=N,'S'=S,'P'=P,'H'=H,'attribute1A'=tasteA,'attribute1B'=tasteB,'attribute2A'=healthA,'attribute2B'=healthB,'RT'=RT,
...
max(ddmRhats) # 1.005
recov_addmJAGS_sp_2Phi$BUGSoutput$DIC # 66031.75


save(recov_addmJAGS_sp_2Phi, file = "param_recov_2phi_sp_food.RData")
```
...
cor(recov_addmJAGS_sp_2Phi$BUGSoutput$mean$sp_H, addmJAGS_sp_2Phi$BUGSoutput$mean$sp+addmJAGS_sp_2Phi$BUGSoutput$mean$sp_H)
cor(recov_addmJAGS_sp_2Phi$BUGSoutput$mean$sp_H+recov_addmJAGS_sp_2Phi$BUGSoutput$mean$weight1, addmJAGS_sp_2Phi$BUGSoutput$mean$weight1)
...
taste_rec <- data.frame(
  rec_sated = recov_addmJAGS_sp_2Phi$BUGSoutput$mean$weight1,
  true_sated = addmJAGS_sp_2Phi$BUGSoutput$mean$weight1,
  rec_hungry = (recov_addmJAGS_sp_2Phi$BUGSoutput$mean$weight1+recov_addmJAGS_sp_2Phi$BUGSoutput$mean$weight1_H),
  true_hungry = addmJAGS_sp_2Phi$BUGSoutput$mean$weight1+addmJAGS_sp_2Phi$BUGSoutput$mean$weight1_H
)
...
theta_rec <- data.frame(
  rec_sated = pnorm(recov_addmJAGS_sp_2Phi$BUGSoutput$mean$theta),
  true_sated = pnorm(addmJAGS_sp_2Phi$BUGSoutput$mean$theta),
  rec_hungry = pnorm(recov_addmJAGS_sp_2Phi$BUGSoutput$mean$theta+recov_addmJAGS_sp_2Phi$BUGSoutput$mean$theta_H),
  true_hungry = pnorm(addmJAGS_sp_2Phi$BUGSoutput$mean$theta+addmJAGS_sp_2Phi$BUGSoutput$mean$theta_H)
)
...
# f) phi 1
phi_rec <- data.frame(
  rec_sated = pnorm(recov_addmJAGS_sp_2Phi$BUGSoutput$mean$phy),
  true_sated = pnorm(addmJAGS_sp_2Phi$BUGSoutput$mean$phy),
  rec_hungry = pnorm(recov_addmJAGS_sp_2Phi$BUGSoutput$mean$phy+recov_addmJAGS_sp_2Phi$BUGSoutput$mean$phy_H),
  true_hungry = pnorm(addmJAGS_sp_2Phi$BUGSoutput$mean$phy+addmJAGS_sp_2Phi$BUGSoutput$mean$phy_H)
)
...
# g) phi 2
phi2_rec <- data.frame(
  rec_sated = pnorm(recov_addmJAGS_sp_2Phi$BUGSoutput$mean$phy2),
  true_sated = pnorm(addmJAGS_sp_2Phi$BUGSoutput$mean$phy2),
  rec_hungry = pnorm(recov_addmJAGS_sp_2Phi$BUGSoutput$mean$phy2+recov_addmJAGS_sp_2Phi$BUGSoutput$mean$phy2_H),
  true_hungry = pnorm(addmJAGS_sp_2Phi$BUGSoutput$mean$phy2+addmJAGS_sp_2Phi$BUGSoutput$mean$phy2_H)
)
...
ggsave("recov_plot_maaddm2phi_food.png", recov_plot_maaddm2phi, width = 18, height = 9)
```
