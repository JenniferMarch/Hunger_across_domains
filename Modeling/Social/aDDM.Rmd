...
T1<-Sys.time()
addmJAGS_noPhi <- jags.parallel(ddmData,inits = NULL,#jags.seed = sample(100:999,1),
                    parameters.to.save = c('mu_bound','sigma_bound','mu_ndt','sigma_ndt',
                                           ...
                                           'mu_theta_H','sigma_theta_H',
                                           'bound','ndt','drift','weight1','theta',
                                           'bound_H','ndt_H','drift_H','weight1_H','theta_H'),
                    model.file = "BayesModel_maaDDM_noPhi.txt",working.directory = 'BayesModels',
                    n.chains = nChains, n.iter = 60000, n.burnin = 30000,n.thin = 12, DIC = TRUE,jags.module = c("glm","dic","wiener"))
T2<-Sys.time() #T2-T1

# check convergence
ddmRhats <- addmJAGS_noPhi$BUGSoutput$summary[,8] #check with max(Rhats), which should ideally be < 1.01 (1.05 would also be okay)
max(ddmRhats)
addmJAGS_noPhi$BUGSoutput$DIC

save.image(file='workspace_addm.RData')

```
## 2.2 Check Parameters (group posteriors)
```{r}
#get group parameter estimates in transformed form 
boundSep_S <- log(1+exp(addmJAGS_noPhi$BUGSoutput$sims.list$mu_bound))
boundSep_H <- log(1+exp(addmJAGS_noPhi$BUGSoutput$sims.list$mu_bound+addmJAGS_noPhi$BUGSoutput$sims.list$mu_bound_H))
boundSep_delta <- addmJAGS_noPhi$BUGSoutput$sims.list$mu_bound_H

ndt_S <- log(1+exp(addmJAGS_noPhi$BUGSoutput$sims.list$mu_ndt))
ndt_H <- log(1+exp(addmJAGS_noPhi$BUGSoutput$sims.list$mu_ndt+addmJAGS_noPhi$BUGSoutput$sims.list$mu_ndt_H))
ndt_delta <- addmJAGS_noPhi$BUGSoutput$sims.list$mu_ndt_H

weight1_S <- pnorm(addmJAGS_noPhi$BUGSoutput$sims.list$mu_weight1)
weight1_H <- pnorm(addmJAGS_noPhi$BUGSoutput$sims.list$mu_weight1+addmJAGS_noPhi$BUGSoutput$sims.list$mu_weight1_H)
weight1_delta <- addmJAGS_noPhi$BUGSoutput$sims.list$mu_weight1_H

drift_S <- log(1+exp(addmJAGS_noPhi$BUGSoutput$sims.list$mu_drift))
drift_H <- log(1+exp(addmJAGS_noPhi$BUGSoutput$sims.list$mu_drift+addmJAGS_noPhi$BUGSoutput$sims.list$mu_drift_H))
drift_delta <- addmJAGS_noPhi$BUGSoutput$sims.list$mu_drift_H

theta_S <- pnorm(addmJAGS_noPhi$BUGSoutput$sims.list$mu_theta)
theta_H <- pnorm(addmJAGS_noPhi$BUGSoutput$sims.list$mu_theta+addmJAGS_noPhi$BUGSoutput$sims.list$mu_theta_H)
theta_delta <- addmJAGS_noPhi$BUGSoutput$sims.list$mu_theta_H
...
tasteweight1<-ggplot(datatw, aes(x = weight1_S)) +
  geom_histogram(binwidth = 1/275, fill = "cornflowerblue", alpha = 0.5, color = "black") +
  geom_histogram(aes(x = weight1_H), binwidth = 1/275, fill = "gold", alpha = 0.5, color = "black") +
...
theta1<-ggplot(datatheta, aes(x = theta_S)) +
  geom_histogram(binwidth = 1/155, fill = "cornflowerblue", alpha = 0.5, color = "black") +
  geom_histogram(aes(x = theta_H), binwidth = 1/155, fill = "gold", alpha = 0.5, color = "black") +
...
ggsave("combined_plot_addm1.png", combined_plot_addm1, width = 16, height = 12)
```
