...
T1<-Sys.time()
addmJAGS_sp_social <- jags.parallel(ddmData,inits = NULL,#jags.seed = sample(100:999,1),
...
# check convergence
ddmRhats <- addmJAGS_sp_social$BUGSoutput$summary[,8] #check with max(Rhats), which should ideally be < 1.01 (1.05 would also be okay)
max(ddmRhats)
addmJAGS_sp_social$BUGSoutput$DIC
#DIC 29612
save.image(file='addm_sp_social.RData')
...
boundSep_S <- log(1+exp(addmJAGS_sp_social$BUGSoutput$sims.list$mu_bound))
boundSep_H <- log(1+exp(addmJAGS_sp_social$BUGSoutput$sims.list$mu_bound+addmJAGS_sp_social$BUGSoutput$sims.list$mu_bound_H))
boundSep_delta <- addmJAGS_sp_social$BUGSoutput$sims.list$mu_bound_H

ndt_S <- log(1+exp(addmJAGS_sp_social$BUGSoutput$sims.list$mu_ndt))
ndt_H <- log(1+exp(addmJAGS_sp_social$BUGSoutput$sims.list$mu_ndt+addmJAGS_sp_social$BUGSoutput$sims.list$mu_ndt_H))
ndt_delta <- addmJAGS_sp_social$BUGSoutput$sims.list$mu_ndt_H

weight1_S <- pnorm(addmJAGS_sp_social$BUGSoutput$sims.list$mu_weight1)
weight1_H <- pnorm(addmJAGS_sp_social$BUGSoutput$sims.list$mu_weight1+addmJAGS_sp_social$BUGSoutput$sims.list$mu_weight1_H)
weight1_delta <- addmJAGS_sp_social$BUGSoutput$sims.list$mu_weight1_H

drift_S <- log(1+exp(addmJAGS_sp_social$BUGSoutput$sims.list$mu_drift))
drift_H <- log(1+exp(addmJAGS_sp_social$BUGSoutput$sims.list$mu_drift+addmJAGS_sp_social$BUGSoutput$sims.list$mu_drift_H))
drift_delta <- addmJAGS_sp_social$BUGSoutput$sims.list$mu_drift_H

theta_S <- pnorm(addmJAGS_sp_social$BUGSoutput$sims.list$mu_theta)
theta_H <- pnorm(addmJAGS_sp_social$BUGSoutput$sims.list$mu_theta+addmJAGS_sp_social$BUGSoutput$sims.list$mu_theta_H)
theta_delta <- addmJAGS_sp_social$BUGSoutput$sims.list$mu_theta_H

sp_S <- pnorm(addmJAGS_sp_social$BUGSoutput$sims.list$mu_sp)
sp_H <- pnorm(addmJAGS_sp_social$BUGSoutput$sims.list$mu_sp+addmJAGS_sp_social$BUGSoutput$sims.list$mu_sp_H)
sp_delta <- addmJAGS_sp_social$BUGSoutput$sims.list$mu_sp_H
...
selfweight1<-ggplot(datatw, aes(x = weight1_S)) +
  geom_histogram(binwidth = 1/130, fill = "cornflowerblue", alpha = 0.5, color = "black") +
  geom_histogram(aes(x = weight1_H), binwidth = 1/130, fill = "gold", alpha = 0.5, color = "black") +
...
theta1<-ggplot(datatheta, aes(x = theta_S)) +
  geom_histogram(binwidth = 1/660, fill = "cornflowerblue", alpha = 0.5, color = "black") +
  geom_histogram(aes(x = theta_H), binwidth = 1/650, fill = "gold", alpha = 0.5, color = "black") +
...
startpoint2<-ggplot(data_deltasp, aes(x = sp_delta)) +
  geom_histogram(binwidth = 1/80, fill = "grey", alpha = 0.5, color = "black") +
  geom_vline(xintercept = 0, color = "red", size = 1.5) +
...
combined_plot_addm2 <- plot_grid(selfweight1, selfweight2, drift1, drift2, nDT1, nDT2, bound1, bound2, theta1, theta2, startpoint1, startpoint2, labels = c("a", "", "b", "", "c", "", "d", "", "e", "", "f", ""), label_size = 20, ncol = 4, align = "h", column_spacing = unit(c(1, 3, 1, 1, 3, 1, 1, 3, 1), "cm"))
combined_plot_addm2
# Save the combined plot as a PNG file
ggsave("addm_sp_social.png", combined_plot_addm2, width = 16, height = 12)
```