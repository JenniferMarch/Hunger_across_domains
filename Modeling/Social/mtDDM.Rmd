...
ddmData <- list('N'=N,'S'=S,'P'=P,'H'=H,'attribute1A'=selfA,'attribute1B'=selfB,'attribute2A'=ngoA,'attribute2B'=ngoB,'RT'=RT,'RT2'=abs(RT))
...
T1<-Sys.time()
mtddm_social <- jags.parallel(ddmData,inits = NULL,#jags.seed = sample(100:999,1),
...
# check convergence
ddmRhats <- mtddm_social$BUGSoutput$summary[,8] #check with max(Rhats), which should ideally be < 1.01 (1.05 would also be okay)
max(ddmRhats)
mtddm_social$BUGSoutput$DIC
#Recommended to save mtddm_social
save.image(file='mtddm_social.RData')

```

## 2.2 maDDM_rst: check parameters (group posteriors)
```{r}
boundSep_S <- log(1+exp(mtddm_social$BUGSoutput$sims.list$mu_bound))
boundSep_H <- log(1+exp(mtddm_social$BUGSoutput$sims.list$mu_bound+mtddm_social$BUGSoutput$sims.list$mu_bound_H))
boundSep_delta <- mtddm_social$BUGSoutput$sims.list$mu_bound_H

ndt_S <- log(1+exp(mtddm_social$BUGSoutput$sims.list$mu_ndt))
ndt_H <- log(1+exp(mtddm_social$BUGSoutput$sims.list$mu_ndt+mtddm_social$BUGSoutput$sims.list$mu_ndt_H))
ndt_delta <- mtddm_social$BUGSoutput$sims.list$mu_ndt_H

weight1_S <- pnorm(mtddm_social$BUGSoutput$sims.list$mu_weight1)
weight1_H <- pnorm(mtddm_social$BUGSoutput$sims.list$mu_weight1+mtddm_social$BUGSoutput$sims.list$mu_weight1_H)
weight1_delta <- mtddm_social$BUGSoutput$sims.list$mu_weight1_H

drift_S <- log(1+exp(mtddm_social$BUGSoutput$sims.list$mu_drift))
drift_H <- log(1+exp(mtddm_social$BUGSoutput$sims.list$mu_drift+mtddm_social$BUGSoutput$sims.list$mu_drift_H))
drift_delta <- mtddm_social$BUGSoutput$sims.list$mu_drift_H

rst_S <- mtddm_social$BUGSoutput$sims.list$mu_rst
rst_H <- mtddm_social$BUGSoutput$sims.list$mu_rst+mtddm_social$BUGSoutput$sims.list$mu_rst_H
rst_delta <- mtddm_social$BUGSoutput$sims.list$mu_rst_H
...
selfweight2<-ggplot(data_deltatw, aes(x = weight1_delta)) +
  geom_histogram(binwidth = 1/150, fill = "grey", alpha = 0.5, color = "black") +
...
rst1<-ggplot(datarst, aes(x = rst_S)) +
  ...
  geom_histogram(aes(x = rst_H), binwidth = 1/20, fill = "gold", alpha = 0.5, color = "black") +
...
combined_plot_mtddm<- plot_grid(selfweight1, selfweight2, drift1, drift2, nDT1, nDT2, bound1, bound2, rst1, rst2, labels = c("a", "", "b", "", "c", "", "d", "", "e", ""), label_size = 20, ncol = 4, align = "h", column_spacing = unit(c(1, 3, 1, 1, 3, 1, 1, 3, 1), "cm"))
combined_plot_mtddm
# Save the combined plot as a PNG file
ggsave("combined_plot_mtddm.png", combined_plot_mtddm, width = 16, height = 12)
