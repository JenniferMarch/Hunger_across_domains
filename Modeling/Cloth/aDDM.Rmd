...
```{r}
# load data
setwd("~/Desktop/WorkingProjects/JM_maaDDM/Hunger_across_domains/")
...
ddmData <- list('N'=N,'S'=S,'P'=P,'attribute1A'=data$upleft_val,'attribute1B'=data$upright_val,'attribute2A'=data$downleft_val,'attribute2B'=data$downright_val,'RT'=data$RT_right/1000,'fixProp1'=data$fixProp1,'fixProp2'=data$fixProp2,'fixProp3'=data$fixProp3,'fixProp4'=data$fixProp4)
...
T1<-Sys.time()
addm_discount <- jags.parallel(ddmData,inits = NULL,#jags.seed = sample(100:999,1),
...
# check convergence
ddmRhats <- addm_discount$BUGSoutput$summary[,8] #check with max(Rhats), which should ideally be < 1.01 (1.05 would also be okay)
max(ddmRhats)
addm_discount$BUGSoutput$DIC

save.image(file='addm_discount.RData')
...
boundSep_S <- log(1+exp(addm_discount$BUGSoutput$sims.list$mu_bound))
boundSep_H <- log(1+exp(addm_discount$BUGSoutput$sims.list$mu_bound+addm_discount$BUGSoutput$sims.list$mu_bound_H))
boundSep_delta <- addm_discount$BUGSoutput$sims.list$mu_bound_H

ndt_S <- log(1+exp(addm_discount$BUGSoutput$sims.list$mu_ndt))
ndt_H <- log(1+exp(addm_discount$BUGSoutput$sims.list$mu_ndt+addm_discount$BUGSoutput$sims.list$mu_ndt_H))
ndt_delta <- addm_discount$BUGSoutput$sims.list$mu_ndt_H

weight1_S <- pnorm(addm_discount$BUGSoutput$sims.list$mu_weight1)
weight1_H <- pnorm(addm_discount$BUGSoutput$sims.list$mu_weight1+addm_discount$BUGSoutput$sims.list$mu_weight1_H)
weight1_delta <- addm_discount$BUGSoutput$sims.list$mu_weight1_H

drift_S <- log(1+exp(addm_discount$BUGSoutput$sims.list$mu_drift))
drift_H <- log(1+exp(addm_discount$BUGSoutput$sims.list$mu_drift+addm_discount$BUGSoutput$sims.list$mu_drift_H))
drift_delta <- addm_discount$BUGSoutput$sims.list$mu_drift_H

theta_S <- pnorm(addm_discount$BUGSoutput$sims.list$mu_theta)
theta_H <- pnorm(addm_discount$BUGSoutput$sims.list$mu_theta+addm_discount$BUGSoutput$sims.list$mu_theta_H)
theta_delta <- addm_discount$BUGSoutput$sims.list$mu_theta_H
