...
T1<-Sys.time()
addm_TD <- jags.parallel(ddmData,inits = NULL,#jags.seed = sample(100:999,1),
...
# check convergence
ddmRhats <- addm_TD$BUGSoutput$summary[,8] #check with max(Rhats), which should ideally be < 1.01 (1.05 would also be okay)
max(ddmRhats)
addm_TD$BUGSoutput$DIC

save.image(file='addm_TD.RData')
...
#parallel computing apparently not possible
T1<-Sys.time()
maaDDM_TD <- jags.parallel(ddmData,inits = NULL,#jags.seed = sample(100:999,1),
parameters.to.save = c('mu_bound','sigma_bound','mu_ndt','sigma_ndt',
                                           'mu_drift','sigma_drift','mu_weight1','sigma_weight1',
...
ddmRhats <- maaDDM_TD$BUGSoutput$summary[,8] #check with max(Rhats), which should ideally be < 1.01 (1.05 would also be okay)
max(ddmRhats)
maaDDM_TD$BUGSoutput$DIC
#max R_hat = 1.032; DIC = 30546.33
save.image(file='maaDDM_TD.RData')
```
...
#parallel computing apparently not possible
T1<-Sys.time()
maa2phiDDM_TD <- jags.parallel(ddmData,inits = NULL,#jags.seed = sample(100:999,1),
parameters.to.save = c('mu_bound','sigma_bound','mu_ndt','sigma_ndt',
                                           'mu_drift','sigma_drift','mu_weight1','sigma_weight1',
...
# n.chains = nChains, n.iter = 60000, n.burnin = 30000,n.thin = 12, 
n.chains = nChains, n.iter = 10000, n.burnin = 1000,n.thin = 2, 
DIC = TRUE,jags.module = c("glm","dic","wiener"))
...
# check convergence and fit
ddmRhats <- maa2phiDDM_TD$BUGSoutput$summary[,8] #check with max(Rhats), which should ideally be < 1.01 (1.05 would also be okay)
max(ddmRhats)
maa2phiDDM_TD$BUGSoutput$DIC
#max R_hat = 1.032; DIC = 30546.33
save.image(file='maa2phiDDM_TD.RData')
```

# 3 Check Parameters (group posteriors)
```{r}
load(file='maa2phiDDM_TD.RData')
```

```{r}
#get group parameter estimates in transformed form 
#ddm_results <- addm_TD
#ddm_results <- maaDDM_TD
ddm_results <- maa2phiDDM_TD
...
est_parameters <- list(
  list(name = "weight1", label = "Amount Weight", binwidth = 1/1000, maxY = 500),
