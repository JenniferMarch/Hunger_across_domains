...
T1<-Sys.time()
addm_social <- jags.parallel(ddmData,inits = NULL,#jags.seed = sample(100:999,1),
...
# check convergence
ddmRhats <- addm_social$BUGSoutput$summary[,8] #check with max(Rhats), which should ideally be < 1.01 (1.05 would also be okay)
max(ddmRhats)
addm_social$BUGSoutput$DIC

save.image(file='addm_social.RData')
...
#parallel computing apparently not possible
T1<-Sys.time()
maaDDM_social <- jags.parallel(ddmData,inits = NULL,#jags.seed = sample(100:999,1),
parameters.to.save = c('mu_bound','sigma_bound','mu_ndt','sigma_ndt',
                                           'mu_drift','sigma_drift','mu_weight1','sigma_weight1',
...
ddmRhats <- maaDDM_social$BUGSoutput$summary[,8] #check with max(Rhats), which should ideally be < 1.01 (1.05 would also be okay)
max(ddmRhats)
maaDDM_social$BUGSoutput$DIC
...
#parallel computing apparently not possible
T1<-Sys.time()
maa2phiDDM_social <- jags.parallel(ddmData,inits = NULL,#jags.seed = sample(100:999,1),
parameters.to.save = c('mu_bound','sigma_bound','mu_ndt','sigma_ndt',
                                           'mu_drift','sigma_drift','mu_weight1','sigma_weight1',
...
# check convergence and fit
ddmRhats <- maa2phiDDM_social$BUGSoutput$summary[,8] #check with max(Rhats), which should ideally be < 1.01 (1.05 would also be okay)
max(ddmRhats)
maa2phiDDM_social$BUGSoutput$DIC
#max R_hat = 1.032; DIC = 30546.33
save.image(file='maa2phiDDM_social.RData')
```

# 3 Check Parameters (group posteriors)
```{r}
load(file='maa2phiDDM_social.RData')
```

```{r}
#get group parameter estimates in transformed form 
ddm_results <- addm_social
...
est_parameters <- list(
  list(name = "weight1", label = "Self Weight", binwidth = 1/500, maxY = 500),
  list(name = "drift", label = "Drift Scaling", binwidth = 1/400, maxY = 500),
  list(name = "boundSep", label = "Boundary Separation", binwidth = 1/300, maxY = 500),
  ...
  list(name = "phy2", label = "phy2", binwidth = 1/800, maxY = 250)
)
