...
2. **Attentional DDM with starting point bias:** Function, Estimate aDDM, Test Parameters and some graphs (corresponds to aDDMsp)
...
```{r}
# Load data
load("data__discount_prep.RData")
```

# 2 Attentional DDM +SP
## 2.1 Estimate maaDDM_sp_2w_noPhi with hierarchical Bayesian modeling (JAGS)
```{r}

# get data and initial values together and specify the model
ddmData <- list('N'=N,'S'=S,'P'=P,'H'=H,'attribute1A'=valueA,'attribute1B'=valueB,'attribute2A'=timeA2,'attribute2B'=timeB2,'RT'=RT,
...
T1<-Sys.time()
aDDMsp_discount <- jags.parallel(ddmData,inits = NULL,#jags.seed = sample(100:999,1),
                    parameters.to.save = c('mu_bound','sigma_bound','mu_ndt','sigma_ndt',
                                           'mu_drift','sigma_drift','mu_weight1','sigma_weight1',
                                           'mu_theta','sigma_theta','mu_sp','sigma_sp',
                                           'mu_bound_H','sigma_bound_H','mu_ndt_H','sigma_ndt_H',
                                           'mu_drift_H','sigma_drift_H','mu_weight1_H','sigma_weight1_H',
                                           'mu_theta_H','sigma_theta_H','mu_sp_H','sigma_sp_H',
                                           'bound','ndt','drift','weight1','theta','sp',
                                           'bound_H','ndt_H','drift_H','weight1_H','theta_H','sp_H'),
                    model.file = "BayesModel_maaDDM_sp_noPhi.txt",working.directory = 'BayesModels',
                    n.chains = nChains, n.iter = 60000, n.burnin = 30000,n.thin = 12, DIC = TRUE,jags.module = c("glm","dic","wiener"))
T2<-Sys.time() #T2-T1

# check convergence
ddmRhats <- aDDMsp_discount$BUGSoutput$summary[,8] #check with max(Rhats), which should ideally be < 1.01 (1.05 would also be okay)
max(ddmRhats)
aDDMsp_discount$BUGSoutput$DIC
#DIC 29612
save.image(file='addm_sp_discount.RData')

```

## 2.2 Check Parameters (group posteriors)
```{r}
#get group parameter estimates in transformed form 
boundSep_S <- log(1+exp(aDDMsp_discount$BUGSoutput$sims.list$mu_bound))
boundSep_H <- log(1+exp(aDDMsp_discount$BUGSoutput$sims.list$mu_bound+aDDMsp_discount$BUGSoutput$sims.list$mu_bound_H))
boundSep_delta <- aDDMsp_discount$BUGSoutput$sims.list$mu_bound_H

ndt_S <- log(1+exp(aDDMsp_discount$BUGSoutput$sims.list$mu_ndt))
ndt_H <- log(1+exp(aDDMsp_discount$BUGSoutput$sims.list$mu_ndt+aDDMsp_discount$BUGSoutput$sims.list$mu_ndt_H))
ndt_delta <- aDDMsp_discount$BUGSoutput$sims.list$mu_ndt_H

weight1_S <- pnorm(aDDMsp_discount$BUGSoutput$sims.list$mu_weight1)
weight1_H <- pnorm(aDDMsp_discount$BUGSoutput$sims.list$mu_weight1+aDDMsp_discount$BUGSoutput$sims.list$mu_weight1_H)
weight1_delta <- aDDMsp_discount$BUGSoutput$sims.list$mu_weight1_H

drift_S <- log(1+exp(aDDMsp_discount$BUGSoutput$sims.list$mu_drift))
drift_H <- log(1+exp(aDDMsp_discount$BUGSoutput$sims.list$mu_drift+aDDMsp_discount$BUGSoutput$sims.list$mu_drift_H))
drift_delta <- aDDMsp_discount$BUGSoutput$sims.list$mu_drift_H

theta_S <- pnorm(aDDMsp_discount$BUGSoutput$sims.list$mu_theta)
theta_H <- pnorm(aDDMsp_discount$BUGSoutput$sims.list$mu_theta+aDDMsp_discount$BUGSoutput$sims.list$mu_theta_H)
theta_delta <- aDDMsp_discount$BUGSoutput$sims.list$mu_theta_H

sp_S <- pnorm(aDDMsp_discount$BUGSoutput$sims.list$mu_sp)
sp_H <- pnorm(aDDMsp_discount$BUGSoutput$sims.list$mu_sp+aDDMsp_discount$BUGSoutput$sims.list$mu_sp_H)
sp_delta <- aDDMsp_discount$BUGSoutput$sims.list$mu_sp_H
...
weight1<-ggplot(datatw, aes(x = weight1_S)) +
  geom_histogram(binwidth = 1/175, fill = "cornflowerblue", alpha = 0.5, color = "black") +
  geom_histogram(aes(x = weight1_H), binwidth = 1/175, fill = "gold", alpha = 0.5, color = "black") +
  labs(x = "Amount Weight", y = "Frequency") +
...
theta1<-ggplot(datatheta, aes(x = theta_S)) +
  geom_histogram(binwidth = 1/700, fill = "cornflowerblue", alpha = 0.5, color = "black") +
  geom_histogram(aes(x = theta_H), binwidth = 1/700, fill = "gold", alpha = 0.5, color = "black") +
...
theta2<-ggplot(data_deltatheta, aes(x = theta_delta)) +
  geom_histogram(binwidth = 1/275, fill = "grey", alpha = 0.5, color = "black") +
  geom_vline(xintercept = 0, color = "red", size = 1.5) +
...
startpoint1<-ggplot(datasp, aes(x = sp_S)) +
  ...
  geom_histogram(aes(x = sp_H), binwidth = 1/150, fill = "gold", alpha = 0.5, color = "black") +
...
startpoint2<-ggplot(data_deltasp, aes(x = sp_delta)) +
  geom_histogram(binwidth = 1/120, fill = "grey", alpha = 0.5, color = "black") +
  geom_vline(xintercept = 0, color = "red", size = 1.5) +
...
combined_plot_addm2 <- plot_grid(weight1, weight2, drift1, drift2, nDT1, nDT2, bound1, bound2, theta1, theta2, startpoint1, startpoint2, labels = c("a", "", "b", "", "c", "", "d", "", "e", "", "f", ""), label_size = 20, ncol = 4, align = "h", column_spacing = unit(c(1, 3, 1, 1, 3, 1, 1, 3, 1), "cm"))
combined_plot_addm2
# Save the combined plot as a PNG file
ggsave("addm_sp_discount.png", combined_plot_addm2, width = 16, height = 12)
```