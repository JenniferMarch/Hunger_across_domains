...
2. **Multi-attribute DDM with starting poing bias:** Function, Estimate DDMsp, Test Parameters and some graphs (corresponds to DDMsp)
...
# 2 Multiattribute DDM +SP
## 2.1 Estimate maDDM_sp_2w with hierarchical Bayesian modeling (JAGS) - estimation
```{r}
...
T1<-Sys.time()
ddmJAGS_sp<- jags.parallel(ddmData,inits = NULL,
                    parameters.to.save = c('mu_bound','sigma_bound','mu_ndt','sigma_ndt',
                                           'mu_drift','sigma_drift','mu_weight1','sigma_weight1','mu_sp','sigma_sp',
                                           'mu_bound_H','sigma_bound_H','mu_ndt_H','sigma_ndt_H',
                                           'mu_drift_H','sigma_drift_H','mu_weight1_H','sigma_weight1_H','mu_sp_H','sigma_sp_H',
                                           'bound','ndt','drift','weight1','sp',
                                           'bound_H','ndt_H','weight1_H','weight2_H','sp_H'),
                    model.file = "BayesModel_maDDM_sp.txt",working.directory = 'BayesModels',
                    n.chains = nChains, n.iter = 60000, n.burnin = 30000,n.thin = 12, DIC = TRUE,jags.module = c("glm","dic","wiener"))
T2<-Sys.time() #T2-T1

# check convergence and fit
ddmRhats <- ddmJAGS_sp$BUGSoutput$summary[,8] #check with max(Rhats), which should ideally be < 1.01 (1.05 would also be okay)
max(ddmRhats)
ddmJAGS_sp$BUGSoutput$DIC

save.image(file="DDMsp_discount2.RData")
```

## 2.2 maDDM_sp_2w: check parameters (group posteriors)
```{r}
#get group parameter estimates in transformed form 
boundSep_S <- log(1+exp(ddmJAGS_sp$BUGSoutput$sims.list$mu_bound))
boundSep_H <- log(1+exp(ddmJAGS_sp$BUGSoutput$sims.list$mu_bound+ddmJAGS_sp$BUGSoutput$sims.list$mu_bound_H))
boundSep_delta <- ddmJAGS_sp$BUGSoutput$sims.list$mu_bound_H

ndt_S <- log(1+exp(ddmJAGS_sp$BUGSoutput$sims.list$mu_ndt))
ndt_H <- log(1+exp(ddmJAGS_sp$BUGSoutput$sims.list$mu_ndt+ddmJAGS_sp$BUGSoutput$sims.list$mu_ndt_H))
ndt_delta <- ddmJAGS_sp$BUGSoutput$sims.list$mu_ndt_H

weight1_S <- pnorm(ddmJAGS_sp$BUGSoutput$sims.list$mu_weight1)
weight1_H <- pnorm(ddmJAGS_sp$BUGSoutput$sims.list$mu_weight1+ddmJAGS_sp$BUGSoutput$sims.list$mu_weight1_H)
weight1_delta <- ddmJAGS_sp$BUGSoutput$sims.list$mu_weight1_H

drift_S <- log(1+exp(ddmJAGS_sp$BUGSoutput$sims.list$mu_drift))
drift_H <- log(1+exp(ddmJAGS_sp$BUGSoutput$sims.list$mu_drift+ddmJAGS_sp$BUGSoutput$sims.list$mu_drift_H))
drift_delta <- ddmJAGS_sp$BUGSoutput$sims.list$mu_drift_H

sp_S <- pnorm(ddmJAGS_sp$BUGSoutput$sims.list$mu_sp)
sp_H <- pnorm(ddmJAGS_sp$BUGSoutput$sims.list$mu_sp+ddmJAGS_sp$BUGSoutput$sims.list$mu_sp_H)
sp_delta <- ddmJAGS_sp$BUGSoutput$sims.list$mu_sp_H
...
tasteweight1<-ggplot(datatw, aes(x = weight1_S)) +
  geom_histogram(binwidth = 1/400, fill = "cornflowerblue", alpha = 0.5, color = "black") +
  geom_histogram(aes(x = weight1_H), binwidth = 1/400, fill = "gold", alpha = 0.5, color = "black") +
...
combined_plot_ddm2 <- plot_grid(tasteweight1, tasteweight2, drift1, drift2, nDT1, nDT2, bound1, bound2, startpoint1, startpoint2, labels = c("a", "", "b", "", "c", "", "d", "", "e", ""), label_size = 20, ncol = 4, align = "h", column_spacing = unit(c(0, 0, 2, 0, 0), "cm"))
combined_plot_ddm2
# Save the combined plot as a PNG file
ggsave("ddmsp_discount.png", combined_plot_ddm2, width = 16, height = 12)
```
