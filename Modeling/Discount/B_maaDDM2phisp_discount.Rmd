...
plot_mirrored_histogram_ci <- function(observed_data, sim_dataframe, bins = 30, show_legend = TRUE) {
  ...
  # Create the plot
  p <- ggplot() +
    # Choice 2 (upper) - Impatient
    geom_col(data = choice2_data,
             aes(x = bin_center, y = observed, fill = "Impatient (data)"),
    ...
    geom_col(data = choice1_data,
             aes(x = bin_center, y = observed, fill = "Patient (data)"),
             width = diff(bin_edges)[1] * 0.8,
             color = "black", alpha = 0.7) +
    ...
    scale_fill_manual(
      name = "Choice",
      values = c("Impatient (data)" = "darkgrey", 
                 "Patient (data)" = "white", 
                 "95% CI (simulated)" = "turquoise"),
      breaks = c("Impatient (data)", "Patient (data)", "95% CI (simulated)")
    ) +
